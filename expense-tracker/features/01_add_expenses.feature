...
Feature: Add Expense
  ...
  Scenario Outline: Successfully add expenses with various valid inputs
    When I add expense with tilte "<title>", amount <amount>, and category "<category>"
    Then the expense should be saved to "data/expenses.json"
    And the expense list would contain an entry with title "<title>", amount <amount>, category "<category>"
  ...
  Scenario: Reject expense with zero amount
    When I try to add an expense with title "Coffee" , amount 0, and category "Bevarages"
    Then the application should raise an error "Amount cannot be empty or 0"
    And no expense should be saved to "data/expenses.json"
    And the application should not crash
  ...
  Scenario: Reject expense with alpha numeric amount
    When I try to add an expense with title "Coffee", amount hg5.00, and category "Bevarages"
  ...
  Scenario: Expense data persist after the application restarts
    ...
    Then the expense list should still contain an entry with title "Dinner", amouint 3.50, category "Food"
